...
winsor_Field_E <- train_winsor$Field_E #separating the columns to ensure only they are transformed
winsor_Pitch_H <- train_winsor$Pitch_H

train_winsor <- subset(train_winsor, select = -Field_E) # Removing from dataframe to be joined in later
train_winsor <- subset(train_winsor, select = -Pitch_H)

transforms_winsor <- data.frame(winsor_field_e, winsor_pitch_h)
...
colnames(trainBC_winsor)[14:15] <- c("Field_E","Pitch_H")
```
...
colnames(test) <- c("Wins","Bat_H","Bat_2B","Bat_3B","Bat_HR","Bat_BB", "Bat_SO","Base_SB","Base_CS","Pitch_H","Pitch_HR","Pitch_BB","Pitch_SO","Field_E","Field_DP")
...
winsor_fe_lambda <- preProcValues_winsor$bc$winsor_field_e$lambda
winsor_ph_lambda <- preProcValues_winsor$bc$winsor_pitch_h$lambda
